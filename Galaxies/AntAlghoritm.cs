...
namespace Galaxies
{
    class AntAlghorithm
    {
        ...
        public List<List<bool>> Algorithm()
        {
            ...
            allLength = allLength / sh.Count / ((sh.Count - 1)/2);
            closeness2 = sh.Count / Volume(allLength);
            ...
            minNumberOfUsing = (int)(totalSteps * a.Count / (ph.Count * (ph.Count - 1) / 2) * closeness2*70.0); //!!следить, где какой count указывать
        ...
        private double Volume(double averageLen)
        {
            ...
            maxLen = maxLen / averageLen; //все это исправить
            vol = 4.0 / 3.0 * Math.PI * Math.Pow(maxLen, 3);
            return vol;
        }
